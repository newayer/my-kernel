...
{
	int value;
	printk("yt8512h_config_initsadfffffffffffffffffffffffffffffffffffffffffffffffff");
	/* Reset phy */
...
{
	int value;
	printk("yt8521sh_config_initsadfffffffffffffffffffffffffffffffffffffffffffffffff");
	/* Reset phy */
	phy_write(phydev, MII_BMCR, BMCR_RESET);
	while (BMCR_RESET & phy_read(phydev, MII_BMCR))
		msleep(30);
...
{
	return phy_drivers_register(motorcomm_driver, ARRAY_SIZE(motorcomm_driver), THIS_MODULE);
}
