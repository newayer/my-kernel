...
static int read_config(void *data)
{
	...
	if(IS_ERR(fp)){
		...
		pos = 0;
		kernel_read(fp, board_config_buf, sizeof(board_config_buf), &pos);
		filp_close(fp,NULL);
			 ...
			 const struct i2c_device_id *id)
{
	...
	xs9922->pad[0].flags = MEDIA_PAD_FL_SOURCE;
	xs9922->pad[1].flags = MEDIA_PAD_FL_SOURCE;
	xs9922->pad[2].flags = MEDIA_PAD_FL_SOURCE;
	xs9922->pad[3].flags = MEDIA_PAD_FL_SOURCE;
	sd->entity.function = MEDIA_ENT_F_CAM_SENSOR;
 
	ret = media_entity_pads_init(&sd->entity, 4, xs9922->pad);
	if (ret < 0)
		goto err_power_off;
#endif
